...
// IronveilBerserker — high speed, reckless
// No block, fast multi-hit pattern: light×3 rapid → heavy finisher
// Pattern B: dash through + hit (unblockable momentum)
...
const MOVE_SPEED   = 110;  // fast
const PATROL_SPEED = 55;

export class IronveilBerserker {
  ...
  constructor(
    ...
    this.sprite = scene.physics.add.image(x, y, 'ironveil_berserker');
  ...
  private _runState(dist: number, delta: number, player: Player): void {
    ...
    switch (this._state) {
      ...
      case 'detect': {
        body.setVelocityX(0);
        if (this._stateTimer > 150) this._transition('approach'); // fast to aggro
        break;
      }
      ...
      case 'attack_light': {
        body.setVelocityX(0);
        if (this._stateTimer > 120) {  // faster than normal
      ...
      case 'attack_unblockable': {
        // Dash-through — reckless momentum hit
      ...
      case 'recover': {
        body.setVelocityX(0);
        if (this._stateTimer > 250) {  // short recovery — stays aggressive
          if (this._hitCount >= 5) this._transition('backoff');
  ...
  private _executeNextInPattern(): void {
    if (this._pattern === 'A') {
      ...
      else { this._patternStep = 0; this._attackTimer = 700; this._transition('recover'); }
    } else {
      // Dash through + unblockable hit
  ...
  receiveHit(result: { data: { damage: number; knockback: number; hitstun: number } }, _attacker: Player): void {
    ...
    // Berserker has no block — takes full damage
    this._hp = Math.max(0, this._hp - result.data.damage);
    this._hitCount++;
    this._attackTimer = 600; // shorter cooldown — stays aggressive
    ...
    this._scene.time.delayedCall(300, () => {  // short stagger — berserker
      if (this._state === 'stagger') this._transition('approach');
    });
  }
  ...
  private _updateFacing(player: Player): void {
    if (this._state !== 'attack_unblockable') {
