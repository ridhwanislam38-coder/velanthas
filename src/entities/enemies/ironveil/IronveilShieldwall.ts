...
// IronveilShieldwall — blocks frontal attacks
// Only vulnerable from behind or from unblockable attacks
// Pattern A: slow advance + shield bash (heavy)
// Pattern B: shield charge → unblockable slam
...
const MOVE_SPEED   = 40;  // slow — it's a wall
const PATROL_SPEED = 25;

export class IronveilShieldwall {
  ...
  constructor(
    ...
    this.sprite = scene.physics.add.image(x, y, 'ironveil_shieldwall');
  ...
  private _runState(dist: number, delta: number, player: Player): void {
    ...
    switch (this._state) {
      ...
      case 'approach': {
        ...
        if (dist > DETECT_RANGE * 1.4) this._transition('patrol');
      ...
      case 'attack_unblockable': {
        // Charge forward with shield — unblockable
        const chargeDir = player.x > this.sprite.x ? 1 : -1;
        body.setVelocityX(160 * chargeDir);
  ...
  receiveHit(result: { data: { damage: number; knockback: number; hitstun: number } }, _attacker: Player): void {
    ...
    // Block frontal hits — only unblockable and rear attacks penetrate
    ...
    const isUnblockable = result.data.knockback >= 200; // unblockable threshold

    if (isFrontal && !isUnblockable) {
      // Blocked — no damage, just chip
      const chip = Math.floor(result.data.damage * 0.05);
      this._hp = Math.max(0, this._hp - chip);
      this._juice.flash(0x4488ff, 0.4, 60);
      return;
    }
