...
// MossWalker — slow, high HP, regenerates 2 HP/sec while on ground
// Pattern A: slow advance → heavy slam
// Pattern B: root grab (unblockable)
...
const DETECT_RANGE  = 160;
const ATTACK_RANGE  = 75;
const MOVE_SPEED    = 35;   // very slow
const PATROL_SPEED  = 20;
const REGEN_PER_SEC = 2;    // HP/sec on ground

export class MossWalker {
  ...
  private _regenAccum = 0; // accumulated regen ms
  ...
  constructor(
    ...
    this.sprite = scene.physics.add.image(x, y, 'moss_walker');
  ...
  update(delta: number, player: Player): void {
    ...
    // Regen — 2 HP/sec while on ground
  ...
  private _tickRegen(delta: number): void {
    const body = this.sprite.body as Phaser.Physics.Arcade.Body;
    ...
    if (this._regenAccum >= 1000) {
      ...
      this._hp = Math.min(this._maxHp, this._hp + REGEN_PER_SEC);
    }
  }
  ...
  private _runState(dist: number, delta: number, player: Player): void {
    ...
    switch (this._state) {
      ...
      case 'attack_unblockable': {
        // Root grab — pulls player
        body.setVelocityX(0);
        if (this._stateTimer > 350) {
          const result = checkHit(this.stance, player.stance, 'heavy');
          if (result.connected) {
            player.receiveHit(40, 50, 35); // low knockback, high hitstun (grabbed)
          }
      ...
      case 'recover': {
        body.setVelocityX(0);
        if (this._stateTimer > 700) { // slow recovery
  ...
  receiveHit(result: { data: { damage: number; knockback: number; hitstun: number } }, _attacker: Player): void {
    ...
    body.setVelocityX(result.data.knockback * 0.6 * kbDir); // reduced KB (heavy)
