...
// GildedEnforcer — armoured, requires 3 hits to break guard
// Guard: first 3 hits absorbed (reduced to 0 damage each), then guard breaks
// Once guard broken: vulnerable, staggered, takes full damage
// Pattern A: guard walk + heavy counter-punch
// Pattern B: guard break → desperate unblockable swing

export type EnemyState =
  | 'patrol' | 'detect' | 'approach'
  | 'telegraph_heavy' | 'telegraph_unblockable'
  | 'attack_heavy' | 'attack_unblockable'
  | 'guard_break' | 'stagger' | 'recover' | 'backoff' | 'dead';
...
const GUARD_HIT_MAX = 3; // hits to break guard

export class GildedEnforcer {
  ...
  // Guard mechanic
  private _guardHitsAbsorbed = 0;
  private _guardBroken = false;
  ...
  constructor(
    ...
    this.sprite = scene.physics.add.image(x, y, 'gilded_enforcer');
  ...
  private _runState(dist: number, delta: number, player: Player): void {
    ...
    switch (this._state) {
      ...
      case 'detect': {
        body.setVelocityX(0);
        if (this._stateTimer > 350) this._transition('approach');
        break;
      }
      ...
      case 'approach': {
        ...
        if (dist < ATTACK_RANGE && this._attackTimer <= 0) {
          this._pattern = this._guardBroken ? 'B' : 'A';
          this._patternStep = 0;
      ...
      case 'recover': {
        body.setVelocityX(0);
        const recoverMs = this._guardBroken ? 600 : 450;
      ...
      case 'backoff': {
        ...
        if (this._stateTimer > 1400) {
          this._hitCount = 0;
          // Restore guard on backoff
          if (this._guardBroken) {
            this._guardBroken = false;
            this._guardHitsAbsorbed = 0;
  ...
  receiveHit(result: { data: { damage: number; knockback: number; hitstun: number } }, _attacker: Player): void {
    ...
    // Guard absorbs first 3 hits (frontal only)
    if (!this._guardBroken) {
      const attackerDir = _attacker.x > this.sprite.x ? 'right' : 'left';
      const facingDir   = this.sprite.flipX ? 'left' : 'right';
      const isFrontal   = attackerDir === facingDir;

      if (isFrontal) {
        this._guardHitsAbsorbed++;
        // Chip flash — blocked
        this._juice.flash(0xffcc00, 0.3, 60);

        if (this._guardHitsAbsorbed >= GUARD_HIT_MAX) {
          // Guard breaks
          this._guardBroken = true;
          this.sprite.setTint(0xff6666); // red tint — broken
          this._juice.shake(0.006, 200);
          this._transition('guard_break');
        }
        return;
      }
    }
    ...
    this._scene.time.delayedCall(80, () => {
      if (this._state !== 'dead') {
        if (this._guardBroken) this.sprite.setTint(0xff6666);
    ...
    this._scene.time.delayedCall(550, () => {
  ...
  get isGuardBroken(): boolean { return this._guardBroken; }
